...
$userid=$_SESSION['id'];
$user=$_SESSION['user'];
echo " <script> alert( ' $user Welcome to sweet Desert ♡ $userid')</script>";
    ...
    <div class="bar"><ul>
            ...
            <li><a href="#">Menu</a>
                <ul>
                    ...
                    <li id="crepe"><a href="crepeal.php">Crepes</a></li>
                </ul>
            </li></ul>
    </div>
    <div class="nbar">
...
<div class="cart show-cart" id="cart">
        ...
        if(mysqli_num_rows($result) < 1){
            ...
            while($row = mysqli_fetch_array($result)){
                ...
                echo"<div class='cart-items' >
                 <form action='homeal.php' method='post'>
        <img class='cart-img' src='imgs/".$row['image']."' alt=''>
    ...
    <!--<a href='#' class='cart-item-delete'><i class='fas fa-trash'></i></a>-->
           ...
           <button style='display: inline; width:0px; height: 0'  name='delete-sweet' id='delete-sweet'>
           <i class='fas fa-trash'></i></button></a>
            ...
            if (!$conn) {
                ...
                if ($result) {
                    ...
                    echo "<script>window.open('homeal.php','_self')</script>";
    ...
    <div>
        ...
        <h3><span class="total">Total</span><?php echo "<span>$total</span>";?><span>₪</span></h3>
        <button class="btn btn1" type="submit" name="deletee"
                id="deletee">Clear Cart</button>
        <a class="btn" href="order.php">Check Out</a>
        </form>
    </div>
    <?php
    $conn = mysqli_connect('localhost','root','','sweetdesert');
    if(!$conn){
        ...
        if(isset($_POST['deletee'])){
            $qqry="DELETE FROM CART WHERE userid=$userid";
            $ress=mysqli_query($conn, $qqry);
            if($ress){echo"<script>alert('You Have Cleared Your Cart');</script>";}
        }
    }
    ?>
</div>
...
<div class="container"><div class="def">
    ...
    <table class="wcm">
        <tr>
            <td><div class="waffle"><h2><a href="waffleal.php">Yummy Waffle!</a></h2> <p>
                    <pre>
                    In Waffle Your Day we have so many yummy waffles
                     order you favourite now! ♡
                    </pre></p></div></td>
            <td><div class="crepe"><h2><a href="crepeal.php">Delicious Crepes!</a></h2> <p>
                    <pre>
                    Here we Have different types of great crepes
                     order you favourite now! ♡
                    </pre></p></div></td>
            <td><div class="make"><h2><a href="makesweetAL.php">Healthy? Make your Sweet</a></h2><p>
                    <pre>
