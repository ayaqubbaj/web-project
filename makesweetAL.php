...
<div class="signn">
    ...
    background-repeat: no-repeat"
         src="" alt=""> -->
    <div class="box">
        <form action="" method="post" class="makesweet">
            <div class="check-box">
                <h3>Base: ♥ 7₪ ♥</h3>
                ...
                <div><button class="btn"
                   name="design" id="design" type="submit" href="#">Order My Design</button></div>
            </div>
...
if(!$conn){
    ...
    if(isset($_POST['design'])){
        ...
        //$syrups .= "| $syrup";
        $chtop= implode(',',$top);
        $chsyrup= implode(',',$syrup);
        $query ="INSERT INTO makesweet
    (userid,username,base,flavours,toppings,syrups)
    values ('$userid','$user','$base','$flavour','$chtop','$chsyrup')";
        $result = mysqli_query($conn,$query);
        if($result){
            echo"<script>alert('Your Design Is Submitted');</script>";
            echo "<script>window.open('makesweetAL.php','_self')</script>";
        }else{
            echo"<script>alert('Your Design Is NOT Submitted');</script>";
        }
    }
}
?>
